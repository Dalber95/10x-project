...
export function createTestSupabaseClient(): SupabaseClient {
  ...
  if (!supabaseUrl || !supabaseKey) {
    throw new Error("Supabase configuration is missing. Make sure SUPABASE_URL and SUPABASE_KEY are set in .env.test");
  }
...
export async function deleteUserFlashcards(userId: string): Promise<number> {
  ...
  const { data, error } = await supabase.from("flashcards").delete().eq("user_id", userId).select("id");
...
export async function deleteUserGenerations(userId: string): Promise<number> {
  ...
  const { data, error } = await supabase.from("generations").delete().eq("user_id", userId).select("id");
...
export async function deleteUserGenerationErrors(userId: string): Promise<number> {
  ...
  const { data, error } = await supabase.from("generation_error_logs").delete().eq("user_id", userId).select("id");
 ...
 */
export async function cleanupRecentTestData(userId: string, minutesAgo = 30): Promise<void> {
  const supabase = createTestSupabaseClient();

...
export class OpenRouterService {
  ...
  private async executeRequest(requestPayload: RequestPayload): Promise<{
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new TimeoutError(`Request timeout after ${this.timeout}ms`)), this.timeout)
        );
    ...
    throw new NetworkError(lastError?.message ?? "Request failed after all retry attempts");
  }
